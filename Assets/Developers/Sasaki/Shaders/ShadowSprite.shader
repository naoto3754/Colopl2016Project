...
////// Lighting:
                float attenuation = 1;
////// Emissive:
                float2 node_3476 = (step(i.uv0.r,_alphathreshold)*i.uv0);
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(node_3476, _MainTex));
                float node_603 = (_MainTex_var.a*_Color.a*i.vertexColor.a); // A
                float3 emissive = ((_MainTex_var.rgb*_Color.rgb*i.vertexColor.rgb*saturate(((attenuation*2.0)+(-1.0))))*node_603);
