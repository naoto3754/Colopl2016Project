...
public class CharacterController : Singlton<CharacterController>
{
    ...
    void Update()
    {
        ...
        if(StageCreater.I.IsPlayingAnimation == false)
        {
            ...
            if (!DummyCardManager.I.CanUseLadder(_DummyCharacter.transform.position))
            {
                deltaVer = -deltaDrop;
            }
            //キャラクターの当たり判定を5点で行う
            ...
            Vector2 moveDir = DummyCardManager.I.CalcAmountOfMovement(new Vector2(deltaHol, deltaVer));
    ...
    private void UpdateCharacterXZPosition(Vector2 moveDir)
    {
        ...
        if(_IsTopOfWall)
            _IsTopOfWall = DummyCardManager.I.OnTopOfWall();
        ...
        if (Mathf.Abs(moveDir.x) > 0f)
        {
            ...
            float foldlineDist = DummyCardManager.I.CalcFoldLineDistance(_DummyCharacter.transform.position, delta);
    ...
    private void UpdateSubTransparent(Vector2 moveDir)
    {
        ...
        float foldlineDist = DummyCardManager.I.CalcFoldLineDistance(_DummyCharacter.transform.position - delta / 2 * Vector3.right, delta);
        foreach (float x in DummyCardManager.I.GetSortXCoordList(_DummyCharacter.transform.position.y))
        {
    ...
    private bool CalcCurrentMoveDirection()
    {
        ...
        foreach (float x in DummyCardManager.I.GetSortXCoordList(charaPos.y))
        {
            if (charaPos.x < x)
                break;
            moveX = !moveX;
