...
public class CharacterController : Singlton<CharacterController>
{
    ...
    void Update()
    {
        ...
        if(StageCreater.I.IsPlayingAnimation == false)
        {
            ...
            if (!DummyCardObjects.I.CanUseLadder(_DummyCharacter.transform.position))
            {
                deltaVer = -deltaDrop;
            }
            //キャラクターの当たり判定を5点で行う
            ...
            Vector2 moveDir = DummyCardObjects.I.CalcAmountOfMovement(new Vector2(deltaHol, deltaVer));
    ...
    private void UpdateCharacterXZPosition(Vector2 moveDir)
    {
        ...
        if(_IsTopOfWall)
            _IsTopOfWall = DummyCardObjects.I.OnTopOfWall();
        ...
        if (Mathf.Abs(moveDir.x) > 0f)
        {
            ...
            float foldlineDist = DummyCardObjects.I.CalcFoldLineDistance(_DummyCharacter.transform.position, delta);
    ...
    private void UpdateSubTransparent(Vector2 moveDir)
    {
        ...
        float foldlineDist = DummyCardObjects.I.CalcFoldLineDistance(_DummyCharacter.transform.position - delta / 2 * Vector3.right, delta);
        foreach (float x in DummyCardObjects.I.GetSortXCoordList(_DummyCharacter.transform.position.y))
        {
    ...
    private bool CalcCurrentMoveDirection()
    {
        ...
        foreach (float x in DummyCardObjects.I.GetSortXCoordList(charaPos.y))
        {
            if (charaPos.x < x)
                break;
            moveX = !moveX;
