...
public class StageCreater : Singlton<StageCreater>
{
    ...
    {
        //ステージの紙オブジェクト生成
        IEnumerable<float> yCoordList = DummyCardManager.I.GetSortYCoordList();
        ...
        foreach (float y in yCoordList)
        {
            ...
            IEnumerable<float> xCoordList = DummyCardManager.I.GetSortXCoordList((prevY + y) / 2);
    ...
    {
        foreach (GameObject decos in DummyCardManager.I.Decoration)
        {
            SetDecoration(decos);
    ...
    private void SetDecoration(GameObject deco)
    {
        ...
        foreach (float x in DummyCardManager.I.GetSortXCoordList(decoPos.y + decoScale.y / 2 * anchorHeightScale))
        {
        ...
        float foldlineDist = DummyCardManager.I.CalcFoldLineDistance(decoAnchorPos, delta);
